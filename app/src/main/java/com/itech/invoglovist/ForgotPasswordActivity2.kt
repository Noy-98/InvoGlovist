...
class ForgotPasswordActivity2 : AppCompatActivity() {
    ...
    private fun checkIfEmailExists(email: String) {
        val usersRef = database.getReference("GuestTbl")
        ...
        usersRef.orderByChild("email").equalTo(email).addListenerForSingleValueEvent(object :
            ...
            override fun onDataChange(snapshot: DataSnapshot) {
                if (snapshot.exists()) {
                    ...
                    Toast.makeText(this@ForgotPasswordActivity2, "Email is not registered in the system", Toast.LENGTH_SHORT).show()
                }
            }

            override fun onCancelled(error: DatabaseError) {
                Toast.makeText(this@ForgotPasswordActivity2, "Failed to check email: ${error.message}", Toast.LENGTH_SHORT).show()
            }
