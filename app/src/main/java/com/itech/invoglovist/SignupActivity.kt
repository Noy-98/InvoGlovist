...
class SignupActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val cashier_first_name = findViewById<TextInputEditText>(R.id.cashier_first_name)
        val cashier_last_name = findViewById<TextInputEditText>(R.id.cashier_last_name)
        ...
        signup.setOnClickListener {
            val fName = cashier_first_name.text.toString()
            val lname = cashier_last_name.text.toString()
            val em = email.text.toString()
            val pass = password.text.toString()
            val cpass = confirm_pass.text.toString()
            val user_type = "Cashier"

            ProgressBar.visibility = View.VISIBLE

            if (fName.isEmpty() || lname.isEmpty() || em.isEmpty() || pass.isEmpty() || cpass.isEmpty()) {
                if (fName.isEmpty()) {
                    cashier_first_name.error = "First Name is required"
                }
                if (lname.isEmpty()) {
                    cashier_last_name.error = "Last Name is required"
                }
                if (em.isEmpty()) {
                ...
                auth.createUserWithEmailAndPassword(em, pass).addOnCompleteListener { task ->
                    if (task.isSuccessful) {
                        ...
                        val databaseRef = database.reference.child("CashierTbl").child(auth.currentUser!!.uid)
